...
if api_key:
    try:
        ...
        # Usar la instrucción del sistema para dotar a Ivo de todas las normas de Oldelval
        system_instruction = (
...
st.markdown("<div class='main-title'>INSPECTAPP v4.0</div>", unsafe_with_html=True)
st.markdown("<div class='main-subtitle'>El Copiloto Digital del Inspector en Campo ─ Versión de Validación de Arquitectura</div>", unsafe_with_html=True)
...
with st.sidebar:
    ...
    # SECCIÓN DEL CHATBOT DE IVO CON POPOVER FLOTANTE
    st.subheader("💬 Asistente Técnico JIT")
    with st.popover("🤖 Hablar con Ivo (Co-piloto)", use_container_width=True):
        st.write("¡Hola! Soy **Ivo**, tu copiloto digital. Preguntame cualquier regla, fórmula o procedimiento de Oldelval, o contame qué tarea vas a inspeccionar hoy para armar el pre-checklist.")
        ...
        if prompt := st.chat_input("Escribile a Ivo..."):
            ...
            # Obtener respuesta del modelo de Gemini
            if model_initialized:
                try:
                    ...
                    response_text = f"Ups, no pude conectarme con mi cerebro de IA: {ex}. Pero puedo recordarte que para excavaciones mayores a 1.20m necesitás doble salida de escape y vigía."
            else:
                # Simulación de respaldo si no hay API Key de Gemini
                response_text = "La API Key de Gemini no está configurada en los Secrets. Sin embargo, como copiloto simulado, te recuerdo que la Tapada Real se calcula como T = D_medida - R_cañería, y que el límite de gases explosivos (L.E.L.) para firmar el permiso de gabinete es estrictamente 0%."
...
st.info("💡 **Objetivo de esta V1:** Validar arquitectura, flujo de toma de decisiones y el comportamiento del copiloto interactivo antes de la prueba piloto final con inspectores en campo.")
...
with tab1:
    st.markdown("<div class='momento-card'>", unsafe_with_html=True)
    st.markdown("<div class='momento-title'>1.1 Verificación Documental (Sertronic y Blister)</div>", unsafe_with_html=True)
    ...
    st.markdown("<div class='momento-card'>", unsafe_with_html=True)
    st.markdown("<div class='momento-title'>1.2 Control Crítico de Gases (Analista de Gases)</div>", unsafe_with_html=True)
    ...
    if bloqueo_gases:
        st.markdown("<div class='hard-gate-alert'>🛑 BLOQUEO ABSOLUTO DE SEGURIDAD (HARD GATE)<br/>" + 
                    "<br/>".join(gases_err) + "</div>", unsafe_with_html=True)
    else:
        st.success("✅ Atmósfera segura. Valores dentro de los rangos tolerados por Oldelval.")
    st.markdown("</div>", unsafe_with_html=True)
    
    st.markdown("<div class='momento-card'>", unsafe_with_html=True)
    st.markdown("<div class='momento-title'>1.3 CPT Simplificado (Firma de Cuadrilla - Opción B)</div>", unsafe_with_html=True)
    st.write("Para evitar sobrecarga administrativa, el inspector certifica mediante Declaración Jurada Solemne que la cuadrilla completa ha firmado la planilla física y tomado conocimiento de los riesgos.")
...
with tab2:
    if not m1_listo:
        ...
        st.markdown("<div class='momento-card'>", unsafe_with_html=True)
        st.markdown("<div class='momento-title'>2.1 Auditoría de Equipamiento e Interferencias en Sitio</div>", unsafe_with_html=True)
        ...
        st.markdown("<div class='momento-card'>", unsafe_with_html=True)
        st.markdown("<div class='momento-title'>2.2 Asistente Matemático de Tapada Real (T) (Norma IT_OL_PDD_04)</div>", unsafe_with_html=True)
        ...
        st.markdown("<div class='momento-card'>", unsafe_with_html=True)
        st.markdown("<div class='momento-title'>2.3 Dinamización por Profundidad (Resolución SRT 503/14 y PO_OL_EIR_08)</div>", unsafe_with_html=True)
        ...
        if bloqueo_fosa:
            st.markdown("<div class='hard-gate-alert'>🛑 BLOQUEO DE EXCAVACIÓN EN FOSA<br/>" + 
                        "<br/>".join(fosa_err) + "</div>", unsafe_with_html=True)
        st.markdown("</div>", unsafe_with_html=True)
        
        st.markdown("<div class='momento-card'>", unsafe_with_html=True)
        st.markdown("<div class='momento-title'>2.4 Posicionamiento de Maquinaria y Bypass de Desvío</div>", unsafe_with_html=True)
        ...
        st.markdown("<div class='momento-card'>", unsafe_with_html=True)
        st.markdown("<div class='momento-title'>2.5 Liberación y Cierre de la Inspección</div>", unsafe_with_html=True)
        if m2_listo:
            st.balloons()
